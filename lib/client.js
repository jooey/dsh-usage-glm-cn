...
function fitChipWidth(chip, onCap) {
  ...
  const fit = () => {
    ...
    onCap(Math.max(0, Math.min(CHIP_MAX_WIDTH, target - 14)));
  };
  ...
  descriptors: [
    {
      ...
      result: {
        ...
        create: () => glmUsageSnapshotResult$schema
      },
...
/** Apply-time client context, kept for the lazy directory fallback below. */
var applyCtx = null;
/** Session id last seen by the slot inject callback. */
var lastSessionId = null;

/** Outer gate: never mount the hook-using chip unless a model directory store is available. */
function GlmUsageChip(props) {
  // The per-session inject callback is cached: if it once ran while the
  // session's model directory was not resolvable yet, the chip would stay
  // hidden forever. Re-resolve lazily at render time before giving up.
  var directory = props.directory;
  if (!directory && applyCtx && lastSessionId != null) {
    try {
      directory = applyCtx.modelDirectories.directoryFor(lastSessionId).store;
    } catch {
      directory = null;
    }
  }
  if (!directory) return null;
  return React.createElement(GlmBalanceChip, Object.assign({}, props, { directory: directory }));
}
...
function GlmBalanceChip(props) {
  ...
  React.useEffect(() => {
    ...
    measure();
    const el = chipRef.current;
    const clip = el ? el.querySelector(".glm-usage-marquee-clip") : null;
